...
--[[
  ...
  for _ =1,num_layers do
    h0t = nn.Linear(feat_size_audio, opt.hidden_size)(h1t)
    h1t = nn.Tanh()(h0t)
  end
  local resultAudio = nn.Linear(opt.hidden_size, opt.output_size)(h1t)

  
  local h0t = nn.Linear(feat_size_audio, opt.hidden_size)(audio)
  local h1t = nn.Tanh()(h0t)
  local resultAudio = nn.Linear(opt.hidden_size, opt.output_size)(h1t)
  
  local h0v = nn.Linear(feat_size_visual, opt.hidden_size)(image)
  local h1v = nn.Tanh()(h0v)
  local resultVisual = nn.Linear(opt.hidden_size, opt.output_size)(h1v)
--]]