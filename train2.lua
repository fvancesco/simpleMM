...
cmd:option('-input_h5_visual','../../mmtwitter/dataset/old/visual_cnn.h5','path to the h5file containing the preprocessed dataset (visual)')
...
cmd:option('-test_size', 100000, 'how many users to use for test set')
...
local function save_output_vectors()
  ...
  --save h5
  local timestap = os.clock()
  local textfile = opt.output_path .. 'text_' .. opt.test_size .. '_'.. opt.num_layers .. '_' .. opt.neg_samples .. '_' .. timestamp'.h5'
  local myFile = hdf5.open(textfile, 'w')
  myFile:write('/tweets', text_out)
  myFile:close()
  local visualfile = opt.output_path .. 'visual_' .. opt.test_size .. '_'.. opt.num_layers .. '_' .. opt.neg_samples .. '_' .. timestamp'.h5'
...
while true do  
    ...
    if iter % opt.print_every == 0 then 
      ...
      if opt.save_output > 0 and combined_rank < best_combined_rank and combined_rank < 600 then --iter > 200 then 
        print('Found better model! Saving h5...')
        best_combined_rank = combined_rank
        save_output_vectors()
      end

      print(string.format('%d  train/val loss: %f/%f  text/visual rank: %.0f(%d)/%.0f(%d) sim: %f  batch/total time: %.4f / %.0f', iter, losses, eval_split('val'), rtext, top5t, rvisual, top5v, sim, time, timeTot/60))
