...
vectors = sys.argv[1]
v4l = int(sys.argv[2])
test_elements = 4649#int(sys.argv[3]) #int(sys.argv[2]) #np.shape(mmX)[0]  #elements to test
...
#output
out = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m2_pred_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
out5 = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m2_pred5_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
out_results = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m2_results_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
...
for i in range(test_elements):
	...
	ss_x = np.argsort(dsims * -1)  #sorted similarity
	closest = ss_x[:k]
	
	#estimated input vector in the other modality
	avg = np.mean(mmY[closest], axis=0)
	#take NN
	dsims = np.dot(mmY, avg.transpose())
