...
#output
out = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m4_pred_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
out5 = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m4_pred5_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
out_results = open("/home/fbarbieri/imagemusic/results/mm_retrieval/"+vectors+"m4_results_"+str(v4l)+"_"+str(test_elements)+".txt", 'w')
...
for i in range(test_elements):
	...
	x = mmX[i,:]
	dsims = np.dot(mmY, x.transpose())
	#dsims[i] = -1 #exclude yourself!
	ss = np.argsort(dsims * -1)  #sorted similarity

	closest = ss[:k]
	
	#take nearest neighbours of closest vector (in the SAME modality)
	predicted[i] = ss[0]
